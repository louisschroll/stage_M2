...
map_risk <- 
  ...
  facet_grid(methods ~ factor(season, levels = c("Winter", "Summer", "Average"))) + #, switch = "y"
    scale_fill_distiller(palette = "Spectral", 
                         ...
                         labels = c("Low", "High")) +
    geom_sf(data = contour_golfe, color = NA, fill = "antiquewhite") +
    geom_sf(data = wind_farm, 
            aes(col = as.factor(zone_type)), fill = NA) +
    geom_sf_text(data = wind_farm, aes(label = zone_nb, col = as.factor(zone_type)), size = 1.4) +
...
risk_by_zone <- sapply(st_intersects(wind_farm, grid_risk_nmix), function(x){grid_risk_nmix$vul_avg_scaled[x]}) %>% 
  map(~{.x %>% as_tibble()}) %>% 
  bind_rows(.id = "Zone") %>% 
  mutate(zone_type = case_when(
  ...
  bind_rows(sapply(st_intersects(wind_farm, grid_risk_occ), function(x){grid_risk_occ$vul_avg_scaled[x]}) %>% 
  map(~{.x %>% as_tibble()}) %>% 
  bind_rows(.id = "Zone") %>% 
  mutate(zone_type = case_when(
    Zone %in% 1:2 ~ "Initial Park Zones",
...
```{r, fig.width=10}
#load("~/stage_M2/1.data/covariates_data_non_scaled.rdata")
grid_risk_nmix$dist_to_shore <- covariates_data$dist_to_shore
grid_risk_occ$dist_to_shore <- covariates_data$dist_to_shore

grid_risk_nmix$bathymetry <- abs(covariates_data$bathymetry)
grid_risk_occ$bathymetry <- abs(covariates_data$bathymetry)

grid_risk_nmix$mean_CHL <- covariates_data$mean_CHL
grid_risk_occ$mean_CHL <- covariates_data$mean_CHL

grid_risk_nmix$mean_SST <- covariates_data$mean_SST
grid_risk_occ$mean_SST <- covariates_data$mean_SST

grid_risk_nmix %>% as_tibble() %>% 
  mutate(vul_scaled = (vulnerability_average - mean(vulnerability_average) / sd(vulnerability_average)),
         vul_scaled2 = c(scale(vulnerability_average))) %>% 
  bind_rows(grid_risk_occ %>% as_tibble() %>% 
              mutate(vul_scaled = (vulnerability_average - mean(vulnerability_average) / sd(vulnerability_average)),
         vul_scaled2 = c(scale(vulnerability_average))),
            .id = "model") %>% 
  mutate(model = ifelse(model == 1, "N-mixture", "Occupancy")) %>% 
  pivot_longer(c(dist_to_shore, bathymetry, mean_CHL, mean_SST), names_to = "Covar") %>% 
  ggplot(aes(x = value, y = vul_scaled2, color = model, fill = model)) +
  facet_wrap(~Covar, scale = "free",
             labeller = labeller(Covar = c(bathymetry = "Depth (m)", 
                                   dist_to_shore = "Distance to the coast (km)", 
                                   mean_CHL = "Chlorophyl A", 
                                   mean_SST = "Sea Surface Temperature (°C)"))) +
  geom_line(alpha = 0.4) +
  geom_smooth() +
  scale_fill_manual(values = c("#9f77b4", "#df9f0a")) +
  scale_color_manual(values = c("#9f77b4", "#df9f0a")) +
  ...
  labs(title = "Relation btw seabird vulnerability and environmental covariates",
       x = "",
       y = "Relative vulnerability",
       color = "Model", fill = "Model")
```


```{r}
grid_vulnerability2 <- grid_risk_nmix %>% 
  select(starts_with("vulnerability")) %>% 
  mutate(#vulnerability_summer = c(scale(vulnerability_summer)),
         #vulnerability_winter = (scale(vulnerability_winter)),
         vulnerability_average = c(scale(vulnerability_average)))
...
legend_position = "right"
plot_title = "Seabird vulnerability in the Gulf of Lion"
plot_title_size = 16
...
map_risk <- 
  ggplot() +
  geom_sf(data = grid_vulnerability2, aes(fill = vulnerability_average), color = NA, lwd = 0) +
  # facet_grid(methods ~ factor(season, levels = c("Winter", "Summer", "Average"))) + #, switch = "y"
    scale_fill_distiller(palette = "Spectral", 
                         breaks = c(min(grid_vulnerability2$vulnerability_average), max(grid_vulnerability2$vulnerability_average)),
                         labels = c("Low", "High")
                         ) +
    guides(
      fill = guide_colourbar(
    ...
    geom_sf(data = wind_farm, 
            aes(col = as.factor(zone_type)), fill = NA) +
    geom_sf_text(data = wind_farm, aes(label = zone_nb, col = as.factor(zone_type)), size = 4) +
    scale_color_manual(values = wind_farm_color) +
    #geom_sf(data = wind_farm, fill = NA, col = "black") +
    labs(title = plot_title) +
    theme_bw() +
...
```{r}
boxplot_risk <- ggplot(data = risk_by_zone %>% filter(method == "N-mixture"), 
                       aes(x = Zone, y = Vulnerability, color = zone_type)) +
  geom_boxplot(linewidth = 0.35) +
  # facet_wrap(~method, scales = "fixed") +
  scale_color_manual(values = wind_farm_color) +
  geom_hline(yintercept = 0, color = "grey", linetype = "dashed") +
