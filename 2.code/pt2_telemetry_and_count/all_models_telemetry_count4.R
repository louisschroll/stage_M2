...
file_rsf_data <- c("1.data/RSF_data_yellow_legged_gull.rdata",
                   "1.data/RSF_data_yellow_legged_gull.rdata",
                   "1.data/RSF_puffin_yelkouan.rdata",
                   "1.data/RSF_puffin_yelkouan.rdata",
                   "1.data/RSF_data_scopoli_shearwaters.rdata",
...
for (i in seq_along(species_vector)){
  ...
  samplesNmixture <- run_Nmixture(data_nmix = data_nmix, n.iter = n.iter, n.burnin=n.burnin, n.chains=n.chains)
  # 2/3 - RSF 
  load(paste0(adress, file_rsf_data[i]))
  data_rsf <- format_rsf_data_for_nimble(df_RSF %>% filter(month %in% month_to_keep[[i]]), covar)
  samplesRSF <- run_RSF(data_rsf, n.iter = n.iter, n.burnin=n.burnin, n.chains=n.chains)
  # 3/3 - Integrated RSF and N-mixture
  samplesint <- run_integrated_Nmix_RSF(data_nmix, data_rsf, n.iter = n.iter, n.burnin=n.burnin, n.chains=n.chains)
  save(samplesNmixture, samplesRSF, samplesint, file = paste0(adress, "3.results/mcmc_outputs/mcmc_output_", species, ".rdata"))
}
