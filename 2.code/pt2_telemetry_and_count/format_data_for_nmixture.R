# HEADER ------------------------------------------------------------------------
#
# Script name:  ~/stage_M2/2.code/pt2_telemetry_and_count/format_data_for_nmixture.R
...
ypel <- get_effectif_and_effort(obs_data=pelmed_obs, 
                                eff_data=pelmed_eff, 
                                gridocc, 
                                sitesocc_id) %>% 
  left_join(y = grid %>% 
...
get_data_for_Nmixture <- function(data_list, grid, species, add_coords=F){
...
# ---- retrieve or compute count data, covariates and coords ----
...
get_effectif_and_effort <- function(obs_data, eff_data, gridocc, sitesocc_id){
  ...
  for (S in session_list){
    ...
    occurence_df <- occurence_df %>% 
      ...
      mutate(y = ifelse(row_number() %in% id_cells_with_obs, effectif, y)) %>% # fill cells with obs w/ effectif
