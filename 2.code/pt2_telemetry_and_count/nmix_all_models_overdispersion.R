...
best_cov <- list(
  ...
  mouette_pygmee_HR = c("log_dist_to_shore", "log_bathymetry", "mean_CHL", "sd_SAL", 
                        "mean_SSH", "log_sd_VEL", "mean_autumn_SST", "mean_winter_SST", 
                        "mean_spring_SST", "mean_summer_SST"),
...
for (i in seq_along(species_vector)){
  ...
  samplesNmixture <- run_Nmixture(data_nmix = data_nmix, 
                                  ...
                                  n.chains = n.chains, 
                                  overdispersion = T)
  save(samplesNmixture, file = paste0(adress, "3.results/mcmc_outputs/Nmix_output_overdispersion_", species, ".rdata"))
}
