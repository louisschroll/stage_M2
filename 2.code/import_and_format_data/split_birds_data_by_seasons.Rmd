...
migralion_obs <- migralion_obs %>% mutate(species_name = ifelse(nom_fr=="fou de bassan",
                                                    #& session %in% c("prenup2022", "prenup2023"), 
                                                    "fou_de_bassan_HR", species_name))
```
...
migralion_obs <- split_periods(migralion_obs, species="goeland leucophee", 
              session_repro = c("prenup2022", "prenup2023"), 
              session_HR = c("postnup2022", "postnup2023"))

pnm_obs <- split_periods(pnm_obs, "goeland leucophee", 
              session_repro = c("2019printemps", "2020printemps", "2021printemps"), 
              session_HR = c("2019automne", "2020automne"))
...
samm_obs <- samm_obs %>% mutate(species_name = ifelse(session %in% c("SAMM1(1winter)", "SAMM2(1winter)") 
...
pnm_obs <- pnm_obs %>% mutate(species_name = ifelse(nom_fr=="mouette melanocephale" 
                                                    & session %in% c("2019automne", "2020automne"), 
...
migralion_obs <- migralion_obs %>% mutate(species_name = ifelse(session %in% c("prenup2022", "prenup2023") 
                                                 & nom_fr == "mouette pygmee",
                                                 "mouette_pygmee_HR", species_name))
```
...
samm_obs <- samm_obs %>% mutate(species_name = ifelse(nom_fr == "mouette rieuse"
                                                      & session %in% c("SAMM1(1winter)","SAMM2(1winter)"),
...
migralion_obs <- migralion_obs %>% 
  mutate(species_name = ifelse(nom_fr=="puffin de scopoli",
                               #& session %in% c("postnup2022", "postnup2023"),
...
```{r}
migralion_obs <- migralion_obs %>% mutate(species_name = ifelse(nom_fr %in% c("labbe pomarin", "labbe parasite")
                                                    & session %in% c("prenup2022", "prenup2023"), 
                                                    "labbe_R", species_name))
```
...
```{r}
migralion_obs <- migralion_obs %>% mutate(species_name = ifelse(nom_fr == "macareux moine", 
                                                    "macareux_moine_HR", species_name))
```
