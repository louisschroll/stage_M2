...
ypel <- grid[pelsites,] %>% 
  mutate(idpel = 1:length(pelsites),
         ytot = 0,
         y2017 = 0,
         y2018 = 0,
         y2019 = 0,
...
for(i in 1:nrow(ypel)){
  ...
  ypel$eff2021[i] <- sum(inteffpel$length[id][year(inteffpel$date)[id] =="2021"])
...
for(i in 1:nrow(intypel)){
  y <-  intypel$year[i]
  ypel[intypel$idpel[i],paste0("y",y)] <- ypel[intypel$idpel[i],paste0("y",y)] + intypel$podSize[i]
  ypel$ytot[intypel$idpel[i]] <- ypel$ytot[intypel$idpel[i]] + intypel$podSize[i] 
}
...
# Run
samplesint <- runMCMC(Cmcmc3, niter = 11000, nburnin = 1000, nchains = 1, samplesAsCodaMCMC = TRUE) ## DT: use runMCMC
...
res_b0int <- samplesint %>%  #rbind(samplesintrand$chain1, samplesintrand$chain2) %>% 
  as_tibble() %>% 
  dplyr::select(starts_with("intIPP"))

res_profint <- samplesint %>%  #rbind(samplesintrand$chain1, samplesintrand$chain2) %>% 
  as_tibble() %>% 
  dplyr::select(starts_with("betaprof_pop"))

res_tauprof21 <- samplesint %>%  #rbind(samplesintrand$chain1, samplesintrand$chain2) %>% 
  as_tibble() %>% 
  dplyr::select(starts_with("tauprof_pop"))


res_qprofint <- samplesint %>%  #rbind(samplesintrand$chain1, samplesintrand$chain2) %>% 
  as_tibble() %>% 
  dplyr::select(starts_with("betaqprof_pop"))


res_dcolint <- samplesint %>%  #rbind(samplesintrand$chain1, samplesintrand$chain2) %>% 
  as_tibble() %>% 
  dplyr::select(starts_with("betadcol_pop"))
...
plot_prof <- tibble(model = "RSF",
  ...
  bind_rows(tibble(mu_prof = res_profipp %>%
                     slice_sample(n = 10000) %>% 
                     pull(`a[2]`), model = "Poisson GLM"),
            #  tibble(mu_prof = res_profint$betaprof_pop, model = "Integrated model"),
            # tibble(mu_prof = res_profRSF21$betaprof_pop, model = "RSF21"),
            tibble(mu_prof = res_profint %>%
                     slice_sample(n = 10000) %>% 
                     pull(betaprof_pop), model = "Integrated model")) %>% 
  ...
  #     seed = 1, width = .1
  #   )) +
  labs(title = "Linear bathymetry effect" ,
       subtitle = "on Sandwich tern space-use",
       x= "",
       y = "") + 
  theme_minimal()+  
  scale_color_manual(values = c("RSF" = red, "Integrated model" = purple, "Poisson GLM" = blue)) +
  scale_fill_manual(values = c("RSF" = red, "Integrated model" = purple, "Poisson GLM" = blue))+
  ...
  guides(color = guide_legend(label.position = "top", keywidth = unit(1, "pt"), nrow =1),
         fill = "none")
...
plot_qprof <- tibble(model = "RSF",
  ...
  bind_rows(tibble(mu_qprof = res_qprofipp$`a[3]`, model = "Poisson GLM"),
  ...
  #     seed = 1, width = .1
  #   )) +
  labs(title = "Quadratic bathymetry effect",
       subtitle = "on Sandwich tern space-use",
       x= "",
       y = "") + 
  theme_minimal()+  
...
plot_dcol <- tibble(model = "RSF",
  ...
  bind_rows(tibble(mu_dcol = res_dcolipp$`a[4]`, model = "Poisson GLM"),
  ...
  #     seed = 1, width = .1
  #   )) +
  labs(title = "Distance to coast effect",
       subtitle = "on Sandwich tern space-use",
       x= "",
       y = "") + 
  theme_minimal() + 
...
gridpred <- sea.gdl2 %>% 
  mutate(dcoast = scale(as.numeric(st_distance(sea.gdl2, st_union(gdlmap))))[,1],
         dist = dcoast) %>% 
...
#   guides(fill = guide_colourbar(title = "Relative space-use",
...
#         plot.subtitle = element_text(family = "Helvetica", hjust = 0.5)) +
#   labs(title = "Poisson GLM")+
#   scale_fill_viridis_c(labels = c("Low", "High"), breaks = c(54.5,163)) +
...
rint_sd <- apply(omint, 1,function(x) { r <- sd(exp(x)) 
return(r)})
rint <- apply(omint, 1 ,function(x) { r <- mean(exp(x)) 
return(r)})
...
pint <- pint_m | pint_sd #  | pint_cv + plot_annotation(title = "SSF - Occupancy model",
#theme = theme(plot.title = element_text(family = "Helvetica", hjust = 0.5, face = "bold"))))
...
pred_precision <- tibble(value = ripp_cv, model = "Poisson GLM") %>% 
  ...
  coord_flip()+
  theme_minimal()+
  labs(title = "Space-use precision",
...
pred_precision 
ggsave(here("Work/RSFIPP/Caugek/ete/predprecision_v1.png"), plot = pred_precision, dpi  =200, width  = 20, height = 15, unit = "cm", bg = "white")
...
pult <- (pleft | pright ) +
  plot_layout(widths = c(2,1), heights = c(1), guide = "keep") +
...
pult
ggsave(here("Work/RSFIPP/Caugek/ete/pglobal_v4.png"), plot = pult, dpi  =300, width  = 50, height = 30, unit = "cm", bg = "white")
