...
test_all_models <- function(cov_combination, data.int){
  ...
  model_result_list <- map(cov_combination, 
                           function(x){run_spOccupancy_models(data.int, selected_cov = x)})
...
run_spOccupancy_models <- function(data.int, selected_cov, add_spatial=FALSE, spatial_model="exponential"){
  if (length(data.int$y) == 1)
    model_result <- run_non_int_model(data.int, selected_cov)
  else
    model_result <- run_int_model(data.int, selected_cov, add_spatial, spatial_model)
  return(model_result)
}
